...
class fergusm5Lab8 : public OpenGLApplicationBase
{
public:
	...
	void setupLighting(GLuint shaderProgram)
	{
		...
		// Get directional light location
		...
		// Get position light location
		GLuint posLightColorLoc = getUniformLocation( shaderProgram,"posLight.colorIntensity");
		GLuint posLightPositionLoc = getUniformLocation( shaderProgram,"posLight.direction");
		...
		//Set position light values
		glUniform4fv(posLightColorLoc, 1, value_ptr(vec4(0.5f, 0.5f, 0.5f, 1.0f)));
		glUniform3fv(posLightPositionLoc, 1, value_ptr(vec3(-2.0f, 2.0f, 0.0f)));
		...
		//Set spition light values
		glUniform4fv(spLightColorLoc, 1, value_ptr(vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		glUniform3fv(spLightPositionLoc, 1, value_ptr(vec3(0.0f, 6.0f, 0.0f)));
		glUniform3fv(spLightDirectionLoc, 1, value_ptr(normalize(vec3(0.0f, -1.0f, 0.0f))));
		glUniform4fv(spLightSpecLoc, 1, value_ptr(vec4(1.0f, 1.0f, 1.0f, 1.0f)));
		glUniform1f(spLightCutoffCosLoc, cos(glm::radians(15.0f)));
		spOn = true;
