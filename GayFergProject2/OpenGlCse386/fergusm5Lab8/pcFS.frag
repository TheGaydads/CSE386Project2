...
void main()
{
	...
	if(spLight.enabled == true) {
		...
		if( spotCosFactor > spLight.spotCutoffCos ) {
			...
			float falloff = 1.0f - (1.0f - spotCosFactor) /
			(1.0f - spLight.spotCutoffCos);
	...
	// Set the fragment to the diffuseMat color. 
		FragColor = object.emissiveMat + totalAmbientLight + totalDirectionLight + totalPositionalLight + totalSpotLight;
